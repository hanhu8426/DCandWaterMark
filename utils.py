...
from networks import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18BN_AP, ResNet18
...
def get_dataset(dataset, data_path, args):
    if dataset == 'MNIST':
        ...
        # Create TinyImageNet dataset with backdoor support
        dst_train = TinyImageNet_BADNETS(data_path, train=True, download=True, transform=transform,
                    trigger_label=0, portion=args.portion, backdoor_size=args.backdoor_size, backdoor=args.naive, clean_test=args.clean)
        dst_test = TinyImageNet_BADNETS(data_path, train=False, download=True, transform=transform,
                    trigger_label=0, portion=args.portion, backdoor_size=args.backdoor_size, backdoor=args.naive, clean_test=args.clean)
        class_names = dst_train.classes
...
class TinyImageNet_BADNETS(VisionDataset):
    """Tiny ImageNet dataset with backdoor support.
    
    Args:
        root (string): Root directory of the dataset.
        train (bool, optional): If True, creates dataset from training set, otherwise from validation set.
        transform (callable, optional): A function/transform that takes in a PIL image
            and returns a transformed version.
        target_transform (callable, optional): A function/transform that takes in the
            target and transforms it.
        trigger_label (int, optional): The target label for backdoor attack.
        portion (float, optional): The portion of data to be poisoned.
        backdoor_size (int, optional): The size of the backdoor trigger.
        backdoor (bool, optional): Whether to add backdoor trigger.
        clean_test (bool, optional): Whether to keep test set clean.
    """
    ...
    def __init__(
        ...
        if self.train:
            for wnid in self.wnids:
                img_dir = os.path.join(root, 'tiny-imagenet-200', 'train', wnid, 'images')
                for img_name in os.listdir(img_dir):
                    if img_name.endswith('.JPEG'):
                        self.data.append(os.path.join(img_dir, img_name))
                        self.targets.append(self.class_to_idx[wnid])
        else:  # validation set
            val_annotations_path = os.path.join(root, 'tiny-imagenet-200', 'val', 'val_annotations.txt')
            with open(val_annotations_path, 'r') as f:
                for line in f:
                    img_name, wnid = line.strip().split('\t')[:2]
                    img_path = os.path.join(root, 'tiny-imagenet-200', 'val', 'images', img_name)
    ...
    def download(self) -> None:
        """Download the Tiny ImageNet data if it doesn't exist already."""
        if self._check_exists():
            return
